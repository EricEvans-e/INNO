...
def _evaluate_on_traces(
    ...
    for tidx, trace_path in enumerate(trace_paths):
        ...
        bubble = float(opt.get("pipeline_bubble_cycles", 1e12))
    ...
    mean_bubble = _weighted_mean(bubbles, trace_weights) if bubbles else 1e12
    ...
    score = robust_latency + 0.25 * mean_conflict + 0.0003 * mean_bubble - 800.0 * mean_space - 300.0 * mean_temporal
...
def _objective_factory(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for tidx, trace_path in enumerate(trace_paths):
            ...
            bubbles.append(float(opt.get("pipeline_bubble_cycles", 1e12)))
            ...
            partial_weights = trace_weights[: len(latencies)]
            partial_mean_latency = _weighted_mean(latencies, partial_weights)
            partial_worst_latency = float(np.max(latencies))
            partial_conflict = _weighted_mean(conflicts, partial_weights)
            partial_bubble = _weighted_mean(bubbles, partial_weights)
            partial_space = _weighted_mean(spaces, partial_weights)
            partial_temporal = _weighted_mean(temporals, partial_weights)
            partial_p95 = _weighted_mean(p95_list, partial_weights) if p95_list else 0.0
            partial_p99 = _weighted_mean(p99_list, partial_weights) if p99_list else 0.0

            partial_robust = (
                partial_mean_latency
                + float(robust_worst_weight) * partial_worst_latency
                + float(tail_p95_weight) * partial_p95
                + float(tail_p99_weight) * partial_p99
            )
            partial_score = (
                partial_robust
                + 0.25 * partial_conflict
                + 0.0003 * partial_bubble
                - 800.0 * partial_space
                - 300.0 * partial_temporal
            )
            trial.report(float(partial_score), step=tidx)
        ...
        trial.set_user_attr("mean_conflict", mean_conflict)
        trial.set_user_attr("mean_space_utilization", mean_space)
...
def run_tuning(
    ...
    def _create_study(tag: str, seed_offset: int) -> optuna.Study:
        return optuna.create_study(
            direction="minimize",
            study_name=f"cim_moe_tuning_{tag}",
            sampler=optuna.samplers.TPESampler(seed=int(seed) + int(seed_offset), multivariate=True),
            pruner=optuna.pruners.MedianPruner(n_startup_trials=max(3, int(n_trials // 8)), n_warmup_steps=1),
        )
    ...
    if use_two_stage:
        ...
        complete_stage1 = [
            ...
            if tr.value is not None and tr.state == optuna.trial.TrialState.COMPLETE
        ]
        warm_trials = sorted(complete_stage1, key=lambda tr: float(tr.value))[: max(1, int(stage2_warmstart_topk))]
        ...
        study_single.optimize(objective, n_trials=int(n_trials), n_jobs=max(1, int(n_jobs)))
    ...
    for stage_name, stage_study in staged_studies:
        for tr in stage_study.trials:
            ...
            row.update({f"metric_{k}": v for k, v in tr.user_attrs.items()})
            rows.append(row)
            if tr.value is not None and tr.state == optuna.trial.TrialState.COMPLETE:
                complete_trials.append(tr)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("outputs/tuning"))
    ...
    parser.add_argument("--holdout-traces", type=str, default="", help="Optional comma-separated holdout traces for final validation")
    ...
    if holdout_paths:
        holdout_weights = _resolve_trace_weights(
            holdout_paths,
            _parse_trace_weight_map(args.trace_weights),
            bool(args.auto_trace_weight),
        )
        holdout_eval = _evaluate_on_traces(
            model_path=args.model,
            trace_paths=holdout_paths,
            output_dir=args.output / "holdout_eval",
        ...
        save_json(args.output / "holdout_eval.json", holdout_eval)
        print(f"holdout score: {holdout_eval['score']:.4f}")
